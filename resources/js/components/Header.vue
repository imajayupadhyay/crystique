<template>
  <header class="fixed top-0 left-0 right-0 z-50 transition-all duration-300" :class="headerScrolled ? 'bg-white/90 backdrop-blur-lg shadow-lg' : 'bg-black/20 backdrop-blur-sm'">
    <nav class="container mx-auto px-6 py-4">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          ...
          <span class="text-2xl font-bold transition-colors duration-300" :class="headerScrolled ? 'bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent' : 'text-white'">
            Crystique
          </span>
        </div>
        ...
        <div class="hidden md:flex items-center space-x-8">
          <Link href="/" class="nav-link font-semibold" :class="[
            isActive('/') ? 'text-purple-600' : (headerScrolled ? 'text-gray-700 hover:text-purple-600' : 'text-white hover:text-purple-300'),
            isActive('/') && 'active'
          ]">Home</Link>
          <Link href="/products" class="nav-link font-semibold" :class="[
            isActive('/products') ? 'text-purple-600' : (headerScrolled ? 'text-gray-700 hover:text-purple-600' : 'text-white hover:text-purple-300'),
            isActive('/products') && 'active'
          ]">Products</Link>
          <Link href="/about" class="nav-link font-semibold" :class="[
            isActive('/about') ? 'text-purple-600' : (headerScrolled ? 'text-gray-700 hover:text-purple-600' : 'text-white hover:text-purple-300'),
            isActive('/about') && 'active'
          ]">About</Link>
          <Link href="/contact" class="nav-link font-semibold" :class="[
            isActive('/contact') ? 'text-purple-600' : (headerScrolled ? 'text-gray-700 hover:text-purple-600' : 'text-white hover:text-purple-300'),
        ...
        <!-- Mobile Menu Button -->
        <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden p-2 rounded-lg transition-all" :class="headerScrolled ? 'hover:bg-gray-100' : 'hover:bg-white/20'">
          <svg class="w-6 h-6" :class="headerScrolled ? 'text-gray-700' : 'text-white'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const isWhiteBackgroundPage = computed(() => {
  return page.url === '/contact' || page.url === '/' || page.url === '/products' || page.url === '/about';
});

// Header should be in scrolled state if either scrolled down or on white background page
const headerScrolled = computed(() => scrolled.value || isWhiteBackgroundPage.value);
